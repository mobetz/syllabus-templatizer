...
public class Recursion {
	public static void main(String[] args) {
        ...
        */
        ...
        String looking_for = "asdfasdfasdf.java";
        ...
        while ( !file_found && places_to_look.size() > 0 ) {
        	...
        	File currently_checked = places_to_look.remove(0);
        	// if the next "place to look" is a file and its name matches the file we're looking for
        	if ( currently_checked.isFile() && currently_checked.getName().equals(looking_for)) {
                file_found = true;//the file was found, yay!
                file_path = currently_checked.getPath();
        	}
        	else if ( currently_checked.isDirectory() ) { //if the place we're checking is a folder
        		for ( File next_file :  currently_checked.listFiles() ) {
        ...
        File result = find_file(initial_location, looking_for);
	...
	public static  int sumArray( List<Integer> numbers_to_add ) {
		...
		//System.out.println("The next time I call this, numbers to add has: " + numbers_to_add);
