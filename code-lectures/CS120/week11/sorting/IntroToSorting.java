...
public class IntroToSorting {
  ...
  public static void main(String[] args) {
    ...
    int[] array = new int[]{1, 2, 3, 4, 5};
          ...
          /*
           To get a pair of positions, I need two different indices. With a single for loop, I've only got
        one index. If I wanted to swap each thing with every other thing exactly once, I'm not just looking
        at each item -- for that item, I'm looking at every single item that comes after it too!

        That means I need a pair of for loops:
          */

          for ( int first_idx = 0; first_idx < array.length -1; first_idx++) {//<- for each thing from the beginning to the end
        ...
        array = new int[]{5, 4, 1, 3, 2};
          ...
          for ( int first_idx = 0; first_idx < array.length -1; first_idx++) {
            ...
            for ( int second_idx= first_idx+1;  second_idx<array.length; second_idx++) { 
              if ( array[first_idx] > array[second_idx] ) {//<- only do the swap if these things are "out of order", i.e if the thing in front is bigger
                swapTwoPlaces(array, first_idx, second_idx);
                System.out.println("Swapping: " + array[first_idx] + " with " + array[second_idx] );
            ...
            System.out.print("After passing through the outer loop once, we have: ");
            printArray(array);
            System.out.println("(Press enter to continue...)");
            in.nextLine();
          }
