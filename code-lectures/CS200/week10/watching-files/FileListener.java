...
public class FileListener implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            this.last_position = 0;  //<- doing this, I will also track "where I last was" in the file, starting at 0
                ...
                for ( WatchEvent<?> event : things_that_happened ) {
                    ...
                    if ( event.kind().equals(StandardWatchEventKinds.ENTRY_MODIFY)) {
                        ...
                        if ( modified_file.equals(this.watched_file.toPath())) {
                            ...
                            //List<String> lines = Files.readAllLines(modified_file);
                            ...
                            long file_remaining = whole_file - this.last_position; //<- this shows how many bytes we HAVEN'T read

                            if ( file_remaining > 0 ) { //<- if there IS anything unread
                                byte[] rest_of_file = new byte[(int) file_remaining];
                                this.open_file.read(rest_of_file); //<- read(bytes[]) modifies the array to read the rest of the file!
