...
public class Apple implements  IDisplayable, ICollidable {
    ...
    public boolean checkCollision(List<ICollidable> others) {
        for (ICollidable other : others) {
            if (this != other && this.x == other.getX() && this.y == other.getY()) {
                this.eaten = true;
                return true;
            }
        }
    ...
    public void destroy() {
        ObjectRegistry.getInstance().destroy((ICollidable) this);
        ObjectRegistry.getInstance().destroy((IDisplayable) this);
    }
}
