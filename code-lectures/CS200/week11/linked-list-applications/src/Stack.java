...
public class Stack<T> {
    ...
    SinglyLinkedNode<T> head; //<- only track the beginning, only move one direction
    ...
    Programmers like to pretend stacks are spring-loaded. We've named the three things a Stack can do accordingly:
        ...
        - "peek" at the thing on front without consuming it
    ...
    public void push(T new_item) {
         ...
         */
        this.head = new SinglyLinkedNode<>(new_item, this.head);
    }
    ...
    public T pop() {
        /*
        For pop, I want to remove the head, return that item, then make the rest of the list
        the new head. This is just peek() but also updating the head:
         */
        T removed_item = null;
        if ( this.head != null ) {
