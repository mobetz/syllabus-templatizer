...
public class AppleSpawner implements IMoveable, IActable {
    ...
    public void act() {
        ...
        for ( Apple apple : apples_made ) {
            if ( apple.isEaten()) {
                ...
                ObjectRegistry.getInstance().destroy((ICollidable) apple);
                ObjectRegistry.getInstance().destroy((IDisplayable) apple);
            }
        }
    ...
    public void move() {
        ...
        while (!moved) {
             ...
             */
            moved = ! ObjectRegistry.getInstance().isOccupied(new_x, new_y);
        }
    }
